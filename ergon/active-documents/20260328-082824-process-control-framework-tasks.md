...
**TypeScript · Mermaid-Extended · Offline-First · Rust-Portable**

A domain-agnostic graph execution engine using annotated Mermaid files as the single-file definition format — topology, typing, node implementation, and execution semantics all in one `.mmd` file. Designed for offline workflow orchestration (Ergon primary), with architecture suitable for later Rust port targeting real-time use.
...
**Graph** — The topology: nodes, typed ports, directed edges, nested subgraphs. Immutable once loaded. Knows nothing about how it will be executed.

**Executor** — The strategy that walks the graph. Swappable per invocation. Four strategies planned:
- *Topological/batch* — dependency-ordered, parallel waves. Primary mode for Ergon.
...
**Context** — Runtime state: the blackboard (scoped shared state), immutable tokens flowing on edges, and execution snapshots for checkpoint/resume. Subgraphs inherit parent context with configurable isolation.
...
Nodes implement a standard async handler interface: `(inputs, context, config) → outputs`. This is the only contract a domain-specific node needs to satisfy — everything else (scheduling, retries, error routing, state persistence) is handled by the framework.

### Data Flow Model

Data moves through the graph in two complementary ways:

- **Edge tokens** — Immutable typed packets that flow along edges from output port to input port. Type-checked at connection time and at runtime. This is the primary data pathway.
- **Blackboard** — Scoped mutable state for cross-cutting concerns (conversation history, accumulated results, shared config). Access is controlled: global, subgraph-local, or node-local scope with read/write permissions.
...
The TypeScript implementation is the working version — optimized for iteration speed and immediate use in Ergon and web-based tooling. The architecture is designed from the start to be portable to Rust: no reliance on dynamic typing in the core, clear separation of the graph model from execution strategy, and an integration test suite defined as graph-plus-expected-output pairs that any implementation must pass.

The Rust port targets three bindings: native library for C/C#/Unity consumers, WASM for browser use, and PyO3 for Python integration. This happens only after the TypeScript version has proven the abstractions across real projects.
...
|----|---|------|-------------------------------|-----|
| 1.1.1 | [ ] | Define core graph types | Node, Edge, Port (input/output), Graph interfaces with generics for payload types | P0 |
| 1.1.2 | [ ] | Implement directed graph with typed ports | Nodes have named, typed input/output ports; edges connect output→input with type validation at connection time | P0 |
...
| 1.1.5 | [ ] | Serialization to/from JSON | Full round-trip: graph topology, port types, node config, subgraph hierarchy | P0 |

### 1.2 Mermaid Integration Layer

| ID | | Task | Details / Acceptance Criteria | Pri |
|----|---|------|-------------------------------|-----|
| 1.2.1 | [ ] | Mermaid flowchart parser | Parse standard Mermaid flowchart syntax into internal graph representation (nodes, edges, subgraphs, labels, edge labels); extract `%% @NodeID` annotations into structured config via dot-path expansion | P0 |
...
| ID | | Task | Details / Acceptance Criteria | Pri |
|----|---|------|-------------------------------|-----|
| 1.3.1 | [ ] | Node lifecycle state machine | States: idle, pending, running, completed, failed, cancelled. Transitions enforced, events emitted | P0 |
| 1.3.2 | [ ] | Topological/batch executor | Resolve dependency order, run nodes sequentially or in parallel waves. Primary mode for Ergon pipelines | P0 |
...
| 1.3.5 | [ ] | Event-driven entry points | External events can push data into designated entry nodes, triggering downstream execution | P1 |
| 1.3.6 | [ ] | Executor strategy as swappable module | Common interface; graph doesn't know which strategy runs it | P0 |
...
| 2.1.2 | [ ] | Parallel node | Run children concurrently; configurable: all-must-succeed, any-can-fail, n-of-m | P0 |
| 2.1.3 | [ ] | Race node | Run children concurrently, resolve on first completion, cancel siblings | P1 |
...
| ID | | Task | Details / Acceptance Criteria | Pri |
|----|---|------|-------------------------------|-----|
| 2.2.1 | [ ] | Typed token flow on edges | Immutable data packets flow along edges; type-checked at connection and runtime | P0 |
| 2.2.2 | [ ] | Blackboard / scoped context | Shared mutable state with scoping (global, subgraph-local, node-local); read/write access control | P0 |
...
| 2.2.4 | [ ] | Execution snapshots | Serialize full execution state (node states, blackboard, pending tokens) to JSON for checkpoint/resume | P1 |
...
| ID | | Task | Details / Acceptance Criteria | Pri |
|----|---|------|-------------------------------|-----|
| 3.1.1 | [ ] | Node type registry | Register handler implementations by type name; lookup at graph load time | P0 |
| 3.1.2 | [ ] | Handler interface | Standard async handler signature: (inputs, context, config) → outputs; with lifecycle hooks (init, cleanup) | P0 |
...
| 3.1.5 | [ ] | Plugin/extension loading | Load node handlers from external packages or file paths at runtime | P2 |
...
| 3.2.2 | [ ] | HTTP / API call node | Method, URL template, headers, body template, response extraction | P1 |
...
| 3.2.5 | [ ] | Human-in-the-loop node | Pause execution, present data, wait for external input, resume with response | P2 |
...
| ID | | Task | Details / Acceptance Criteria | Pri |
|----|---|------|-------------------------------|-----|
| 4.1.1 | [ ] | Execution event bus | Emit structured events for: node state changes, data flow, errors, timing. Subscribe/unsubscribe | P0 |
| 4.1.2 | [ ] | Structured logging | Per-node log context (node ID, execution ID, timestamp); configurable verbosity | P1 |
...
| ID | | Task | Details / Acceptance Criteria | Pri |
|----|---|------|-------------------------------|-----|
| 4.2.1 | [ ] | CLI runner | Load graph from annotated `.mmd` file, execute, output results. Ergon's primary entry point | P0 |
...
| 4.2.4 | [ ] | Visual debugger | Step through execution node-by-node; inspect context/blackboard at each step. Web-based UI | P2 |

---

## Phase 5 — Portability & Future-Proofing

### 5.1 Architecture for Rust Port

| ID | | Task | Details / Acceptance Criteria | Pri |
|----|---|------|-------------------------------|-----|
| 5.1.1 | [ ] | Document core API surface | Freeze and document the public API that must be preserved in any port | P1 |
| 5.1.2 | [ ] | Identify Rust-unfriendly patterns | Audit for shared mutable state, dynamic typing, closures-as-config; document migration strategies | P1 |
| 5.1.3 | [ ] | Integration test suite | Comprehensive tests defined as graph + expected output pairs; portable across implementations | P1 |
| 5.1.4 | [ ] | Rust port of core graph + topological executor | petgraph-based, with typed ports via traits/generics, arena-allocated blackboard | P2 |
| 5.1.5 | [ ] | WASM compilation target | Compile Rust core to WASM for web use; JS bindings via wasm-bindgen | P2 |
| 5.1.6 | [ ] | C FFI for Unity integration | Expose core engine as C-compatible shared library for Hot City and other native consumers | P2 |
...
| **P2** | Future / nice-to-have. Real-time use cases, visual tooling, Rust port. |
