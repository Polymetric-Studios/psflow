...
- [x] Resolve the §6 feasibility question for `claude_workflow` (headless / Agent SDK invocation). **Done** — resolved to headless `claude -p` (see §4.3.1). Remaining for this item:
  ...
  - [x] Dialog routing seam — `Defer` choice + `ApprovalNotifier` (prompt + remote-control URL) + handler `approval: "notify"`. Verified live end-to-end: dialog routed with its real URL, a human opened it and answered, the turn resumed and completed (`file_created=true`). Also added in-session file-channel routing (`approval: "ask"`): dialog written to a request file, blocks on a response file — verified end-to-end through `graphs/claude-ask.mmd` (answered by writing `allow` from the controlling session). Remaining (optional): an MCP round-trip `Custom` policy (same pattern, ergon channel instead of files) so the user's main session answers natively.
...
- Dialog routing — `ApprovalChoice::Defer`, an `ApprovalNotifier` hook (`set_approval_notifier`) fired once per dialog with the prompt + the session's `remote_control_url` (scraped from screen), and drive-loop wiring (route once → on `Defer` wait for an external actor to clear it). Handler `approval: "notify"` defers + logs the prompt/URL via tracing. The same seam serves MCP/Slack (any `Custom` policy can block on a channel). Verified live (`examples/pty_notify.rs`): a forced dialog was routed with its real `https://claude.ai/code/session_…` URL; a human opened it, answered Yes, the dialog cleared, the turn resumed and completed (`file_created=true`, result via transcript). Full human-in-the-loop remote-control routing confirmed end-to-end. 26 terminal unit tests.
- In-session (file-channel) routing — `claude_workflow` `approval: "ask"`: parks the dialog, writes the prompt to `<tmpdir>/psflow-approvals/<node>.request.json`, and blocks polling `<node>.response` for a written choice (`allow`/`deny`/`<n>`); `parse_choice` maps it; deadline/cancel → Deny. Lets the surrounding process/operator answer in-place (no remote-control URL). Verified end-to-end through a graph (`graphs/claude-ask.mmd`): dialog routed into the controlling session, answered by writing `allow`, file created, turn completed (`source=transcript`). This is the same pattern an MCP round-trip would use (swap the file channel for an ergon channel).
