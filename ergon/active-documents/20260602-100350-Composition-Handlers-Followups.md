...
- [x] Resolve the §6 feasibility question for `claude_workflow` (headless / Agent SDK invocation). **Done** — resolved to headless `claude -p` (see §4.3.1). Remaining for this item:
  ...
  - [x] Dialog routing seam — `Defer` choice + `ApprovalNotifier` (prompt + remote-control URL) + handler `approval: "notify"`. Verified live (routed with real URL, deferred, timed out). Remaining: a human-click test that opening the URL clears the dialog; an MCP round-trip `Custom` policy (callback blocking on an ergon channel) so the user's main session answers.
