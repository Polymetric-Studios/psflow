...
**Current blocker (Composio-side maintenance).** As of 20260601 the live trigger could not be created. Root cause, after isolating: Composio is performing **maintenance on this project** (dashboard Playground shows "temporarily disabled… performing maintenance"). All failures were *project-scoped*, consistent with that:
- Dashboard **`ak_` project key** → 401 "Invalid API key" (correct 23-char `ak_…` format per docs — not a length/keychain issue; both were ruled out).
- `composio dev init` → "Failed to list org projects: HTTP 404".
- The CLI's **`uak_` user key** (47 chars, in `~/.composio/user_data.json`) authenticates fine via the **`x-user-api-key`** header (REST returned 200) — this is why tool execution and all manual/scheduled workflows keep working.
- The `@composio/core` SDK only sends `x-api-key`, so it cannot use the `uak_` key (injecting `x-user-api-key` via `defaultHeaders` still 401s on the rejected `x-api-key`). The SDK therefore needs a working `ak_` project key, which is down for maintenance.

Resume test (run after maintenance clears): `GET /api/v3/toolkits` with the `ak_` key via `x-api-key` → expect 200. Then `composio dev init` should also work, and `just trigger-create …` / `just triggers …` go live. If still 401 after maintenance ends, it's a real provisioning issue → Composio support.
