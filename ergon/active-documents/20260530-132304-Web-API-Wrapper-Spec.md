...
- **Auth needs cookie + `x-xsrf-token`, exceeding the built-in `cookie_jar`.** Recon found the generate POST requires an `x-xsrf-token` header whose value is the `XSRF-TOKEN` cookie. `cookie_jar` injected cookies but did not echo a cookie into a header. Resolved (psflow gap #1): `cookie_jar` now echoes a named cookie into a header via `csrf_cookie`/`csrf_header` (URL-decoded). GET operations (`list_models`, `history`) need only the cookie.
- **Completion needs a reactive WS handshake (Pusher), exceeding static `init_frames`.** Resolved (psflow gap #2): the `ws` handler now supports `config.handshake` — on a triggering frame it optionally calls an auth endpoint and sends a computed frame, expressing the Pusher `connection_established` → `broadcasting/auth` → `subscribe` flow.
